...
function analyseHeadingOrder($: ReturnType<typeof load>): HeadingOrderAnalysis {
...
export default defineEventHandler(async (event) => {
    ...
    const contents = await axios.get(query.url as string);

    if (contents.status !== 200) {
        throw createError({
            statusCode: 500,
            statusMessage: `Failed to fetch page ${query.url as string}`,
            data: contents.data,
        });
    }

    const $ = load(contents.data);
    const title = $("title").text();
    const headingAnalysis = analyseHeadingOrder($);

    return {
        url: query.url as string,
        title,
        headingOrder: headingAnalysis,
    };
});